...
<div class="container-fluid pt-3" id="contact-panel">
	...
	<ul class="list-group" id="convo-list">
		<?php
		$thread = $conn->query("SELECT * from thread where concat('[',REPLACE(user_ids,',','],['),']') like '%[{$_SESSION['user_id']}]%' ");
		while ($row = $thread->fetch_array()) :
			$user = $conn->query("SELECT * FROM users where id in ({$row['user_ids']}) and id!= {$_SESSION['user_id']} ")->fetch_array();
		...
		?>
			<li class="list-group-item w-100" data-id="<?php echo md5($row['id']) ?>">
				<table class="" width="100%">
					<tr>
						<td width="10%">
							<div class="uavatar"><img src="assets/uploads/<?php echo $user['avatar'] ?>" alt=""></div>
						</td>
...
<ul id="clone_ul" style="display: none">
	<li class="list-group-item w-100">
		<table class="" width="100%">
			<tr>
				<td width="10%">
					<div class="uavatar"><img src="" alt=""></div>
				</td>
...
<script>
	...
	websocket.onmessage = function(e) {
		...
		if (data == null)
			return false;
		console.log(data)
		if (data.type == "chat_sent" && data.by != "<?php echo $_SESSION['user_id'] ?>") {
	...
	function renew_contacts(data) {
		...
		var mid = '<?php echo $_SESSION['user_id'] ?>';
